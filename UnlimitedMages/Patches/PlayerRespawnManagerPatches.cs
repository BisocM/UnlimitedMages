...
namespace UnlimitedMages.Patches
{
    ...
    public static class PlayerRespawnManagerPatches
    {
        ...
        public static void OnStartClient_Postfix(PlayerRespawnManager __instance)
        {
            ...
            float[] originalPositions = (float[])positionsField.GetValue(__instance);
            ...
            float firstPos = GameConstants.PlayerRespawnManager.DeathMessageFirstY;
            float lastPos = GameConstants.PlayerRespawnManager.DeathMessageLastY;
            float totalRange = firstPos - lastPos;
            float newSpacing = totalRange / (newLobbySize - 1);

            for (int i = 0; i < newLobbySize; i++)
            {
                newPositions[i] = firstPos - (i * newSpacing);
            }

            positionsField.SetValue(__instance, newPositions);

            // Update the 'warlocksset' field, which may be used in game logic to determine team size.
        ...
        public static IEnumerable<CodeInstruction> FadeInVignette_Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            {
                // Target: ldc.i4.4 followed by stfld warlocksset
                if (newInstructions[i].opcode != OpCodes.Ldc_I4_4 || newInstructions[i + 1].opcode != OpCodes.Stfld) continue;

                var fieldInfo = newInstructions[i + 1].operand as FieldInfo;
                if (fieldInfo == null || fieldInfo.Name != GameConstants.PlayerRespawnManager.WarlocksSetField) continue;
        ...
        public static void EndGame_Postfix(PlayerRespawnManager __instance)
        {
            ...
            GameObject scoreboard = (GameObject)scoreboardField.GetValue(__instance);
            if (scoreboard == null) return;

            if (UnlimitedMagesPlugin.TeamSizeConfig?.Value == null) return;
            int requiredChildCount = UnlimitedMagesPlugin.TeamSizeConfig.Value * GameConstants.Game.NumTeams;

            Transform scoreboardTransform = scoreboard.transform;
            int currentChildCount = scoreboardTransform.childCount;
            if (currentChildCount >= requiredChildCount) return;
            if (currentChildCount == 0) return;

            GameObject template = scoreboardTransform.GetChild(0).gameObject;

            for (int i = currentChildCount; i < requiredChildCount; i++)
            {
                GameObject newPanel = Object.Instantiate(template, scoreboardTransform);
        ...
        public static bool PlayAnnouncerSound_Prefix(PlayerRespawnManager __instance, int pteam)
        {
            ...
            foreach (GameObject deadPlayer in deadPlayers)
            {
                ...
                if (pm.playerTeam == 0) team1Deaths++;
                else if (pm.playerTeam == 2) team2Deaths++;
            }
            ...
            switch (pteam)
            {
                case 0 when team1Deaths > 0:
                {
                    ...
                    int clipIndex = Mathf.Clamp(team1Deaths - 1, 0, GameConstants.Game.OriginalTeamSize - 1);
                    serverPaSoundMethod.Invoke(__instance, new object[] { clipIndex });
                    break;
                }
                case 2 when team2Deaths > 0:
                {
                    int clipIndex = Mathf.Clamp(team2Deaths - 1, 0, GameConstants.Game.OriginalTeamSize - 1);
                    serverPaSoundMethod.Invoke(__instance, new object[] { clipIndex + GameConstants.Game.OriginalTeamSize });
                    break;
                }
            }
